...
describe("OpenResty native proxy configuration", function()
  ...
  local api_proxy_path = os.getenv("STACKVERSE_API_PROXY") or "/etc/nginx/templates/api-proxy.conf"
  ...
  local api_proxy = read(api_proxy_path)
  ...
  it("runs gateway policy in the access phase", function()
    assert.is_truthy(api_proxy:match('access_by_lua_block%s*{.-require%("stackverse.api"%)%.prepare%(%)'))
    assert.is_truthy(template:match('access_by_lua_block%s*{.-require%("stackverse.spa"%)%.prepare%(%)'))
  ...
  it("uses native proxy_pass for backend and frontend upstreams", function()
    assert.is_truthy(api_proxy:find("proxy_pass $stackverse_backend_url;", 1, true))
  ...
  it("strips browser credentials before proxying", function()
    ...
    assert.is_truthy(api_proxy:find("proxy_set_header Authorization $stackverse_authorization;", 1, true))
  end)

  it("verifies upstream TLS certificates", function()
    assert.is_truthy(common:find("proxy_ssl_verify on;", 1, true))
    assert.is_truthy(common:find("proxy_ssl_trusted_certificate /etc/ssl/certs/ca-certificates.crt;", 1, true))
  end)
end)
