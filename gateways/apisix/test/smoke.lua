...
local function test_config_defaults()
  ...
  assert_equal(cfg.oidc.session_contents.refresh_token, true)
...
local function test_spa_static_and_proxy_modes()
  local root = "/tmp/stackverse-apisix-spa-test"
...
local function test_apisix_plugin_dispatch()
  local calls = {}
  local function handler(name)
    return {
      [name == "readyz" and "check" or "handle"] = function()
        calls[#calls + 1] = name
        return name
      end,
    }
  ...
  with_modules({
    ...
    ["stackverse.readyz"] = handler("readyz"),
    ["stackverse.security"] = {
      apply_headers = function()
        calls[#calls + 1] = "headers"
      end,
      issue_csrf_cookie = function()
        calls[#calls + 1] = "csrf"
      end,
    },
    ["stackverse.spa"] = handler("spa"),
    ["resty.session"] = {
      init = function()
        calls[#calls + 1] = "session-init"
      end,
    },
  }, {
    "apisix.core",
    ...
    assert_equal(plugin.check_schema({}), true)
    plugin.init_worker()
    assert_equal(calls[#calls], "session-init")

    set_ngx({ var = { uri = "/" } })
    assert_equal(plugin.access(), "spa")
    set_ngx({ var = { uri = "/api/v1/me" } })
    assert_equal(plugin.access(), "api")
    set_ngx({ var = { uri = "/readyz" } })
    assert_equal(plugin.access(), "readyz")
    set_ngx({ var = { uri = "/auth/session" } })
    assert_equal(plugin.access(), "session")

    set_ngx({ var = { uri = "/healthz" } })
    assert_equal(plugin.access(), 200)
    assert_equal(ngx.header["Content-Type"], "text/plain; charset=utf-8")

    set_ngx({ var = { uri = "/" } })
    plugin.header_filter()
    assert_equal(calls[#calls], "headers")
  end)
end
