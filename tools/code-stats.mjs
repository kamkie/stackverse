...
// no lines and is not counted as a source file. Hidden dotfiles and
// dot-directories (.yarn/, .config/, .mvn/, .gitignore, .yarnrc.yml, ...) are
// excluded — this keeps vendored tooling such as .yarn/sdks/typescript/lib/*.js
// out of the counts, at the cost of not counting a little dotfile config.
//
...
const TEST_JVM = ['*Test.kt', '*Test.java', '*Test.scala', '*Test.groovy',
  ...
  '*Spec.kt', '*Spec.java', '*Spec.scala', '*Spec.groovy', '*IT.kt', '*IT.java', '*IT.scala'];
...
function collect(selectedLayers) {
  ...
  for (const f of tracked) {
    ...
    if (variant.startsWith('.')) continue; // no dot-directory masquerading as a variant
  ...
  for (const r of reports) {
    ...
    if (parts.length < 3) continue;
    // Exclude hidden dotfiles and dot-directories (.yarn/, .config/, .mvn/, ...).
    // tokei skips these by default; enforcing it here keeps vendored tooling such
    // as .yarn/sdks/typescript/lib/typescript.js out of the counts regardless of
    // how tokei is invoked. A little dotfile config is therefore uncounted — that
    // is deliberate.
    if (parts.some((seg) => seg.startsWith('.'))) continue;
    const [layer, variant] = parts;
...
function renderMarkdown(records) {
  ...
  out.push('ignores, or vendored caches; hidden dotfiles and dot-directories such as');
  out.push('`.yarn/` are excluded); line counts and languages come from');
...
function renderJson(records) {
  const variants = records.map((r) => ({
    ...
    metrics: {
      ...
      testToSource: r.srcLoc > 0 ? Number(r.testToSource.toFixed(4)) : null,
