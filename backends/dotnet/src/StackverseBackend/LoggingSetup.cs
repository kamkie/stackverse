...
public static class LoggingSetup
{
    public static void Configure(WebApplicationBuilder builder)
    {
        ...
        // `debug` lifts the cap for auth/EF diagnosis, `error` tightens it further.
        if (level is LogLevel.Debug or LogLevel.Error)
        {
            builder.Configuration["Logging:LogLevel:Microsoft.AspNetCore"] = level.ToString();
            builder.Configuration["Logging:LogLevel:Microsoft.EntityFrameworkCore"] = level.ToString();
            // appsettings caps SQL-command logging at Critical (EF error-logs its expected
            // first-boot probe for __EFMigrationsHistory); `debug` lifts that cap too
            builder.Configuration["Logging:LogLevel:Microsoft.EntityFrameworkCore.Database.Command"] = level.ToString();
        }
