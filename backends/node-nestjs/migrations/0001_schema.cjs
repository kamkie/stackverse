/**
 * The whole schema in one migration — this backend owns its database
 * (backends/README.md: swapping backends means a fresh database).
 *
 * Deliberate storage idioms vs the reference backend: enum-like columns hold
 * the lowercase wire values ('public', 'open', ...) directly, and tags are a
 * PostgreSQL text[] with a GIN index instead of a join table — same wire
 * behavior, different storage shape (comparison material, see the README).
 */
