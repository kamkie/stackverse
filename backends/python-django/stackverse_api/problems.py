...
def json_response(payload: Any, status: int = 200, headers: dict[str, str] | None = None) -> HttpResponse:
...
def _values(request: Any, name: str) -> list[str]:
    if hasattr(request, "query_params"):
        return [str(value) for value in request.query_params.getlist(name)]
    if isinstance(request, HttpRequest):
        return [str(value) for value in request.GET.getlist(name)]
    return []


def single_param(request: Any, name: str) -> str | None:
    values = _values(request, name)
    if not values:
        return None
    if len(values) > 1:
...
def first_param(request: Any, name: str) -> str | None:
    values = _values(request, name)
    return values[0] if values else None


def multi_param(request: Any, name: str) -> list[str]:
    return _values(request, name)


def require_valid_paging(request: Any) -> tuple[int, int]:
