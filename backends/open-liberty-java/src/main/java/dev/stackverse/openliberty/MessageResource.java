...
public class MessageResource extends ResourceSupport {
  ...
  public Response list() throws SQLException {
    ...
    ResponsePage result = queryPage(
        ...
        paging,
        this::message);
    return JsonSupport.etagResponse(headers.getHeaderString("If-None-Match"), page(result.items(), paging, result.total()));
  }
  ...
  public Response bundle() throws SQLException {
    ...
    Map<String, String> messages = MessageCatalog.bundle(language);
    Response response = JsonSupport.etagResponse(
        headers.getHeaderString("If-None-Match"),
        linked("language", language, "messages", messages));
