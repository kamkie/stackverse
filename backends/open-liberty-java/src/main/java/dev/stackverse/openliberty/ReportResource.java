...
public class ReportResource extends ResourceSupport {
  ...
  public Response create(@PathParam("id") String rawBookmarkId, String body) {
  ...
  public Response listMine() throws SQLException {
    ...
    ResponsePage result = reportPage(where, params, paging, "order by created_at desc, id desc");
    return JsonSupport.json(page(result.items(), paging, result.total()));
  }

  @PUT
  @Path("/api/v1/reports/{id}")
  public Response updateMine(@PathParam("id") String rawId, String body) {
    Caller caller = requireCaller();
    UUID id = uuid(rawId);
