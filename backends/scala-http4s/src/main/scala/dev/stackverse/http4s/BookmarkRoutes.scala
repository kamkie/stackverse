...
final class BookmarkRoutes(db: Db, auth: AuthService, repository: SharedRepository, handler: RequestHandler) {
  ...
  val routes: HttpRoutes[IO] = HttpRoutes.of[IO] {
    case req @ GET -> Root / "api" / "v1" / "bookmarks"         => handler(req)(listBookmarksV1(req))
    case req @ POST -> Root / "api" / "v1" / "bookmarks"        => handler(req)(createBookmark(req))
    case req @ GET -> Root / "api" / "v2" / "bookmarks"         => handler(req)(listBookmarksV2(req))
    case req @ GET -> Root / "api" / "v1" / "bookmarks" / id    => handler(req)(getBookmark(req, id))
    case req @ PUT -> Root / "api" / "v1" / "bookmarks" / id    => handler(req)(updateBookmark(req, id))
    case req @ DELETE -> Root / "api" / "v1" / "bookmarks" / id => handler(req)(deleteBookmark(req, id))
    case req @ GET -> Root / "api" / "v1" / "tags"              => handler(req)(listTags(req))
  }
