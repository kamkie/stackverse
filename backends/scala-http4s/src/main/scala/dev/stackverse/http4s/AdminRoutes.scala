...
final class AdminRoutes(
  ...
  val routes: HttpRoutes[IO] = HttpRoutes.of[IO] {
    case req @ GET -> Root / "api" / "v1" / "admin" / "users"            => handler(req)(listUsers(req))
    case req @ GET -> Root / "api" / "v1" / "admin" / "users" / username => handler(req)(getUser(req, username))
    case req @ PUT -> Root / "api" / "v1" / "admin" / "users" / username / "status" =>
      handler(req)(setUserStatus(req, username))
    case req @ GET -> Root / "api" / "v1" / "admin" / "audit-log" => handler(req)(auditLog(req))
    case req @ GET -> Root / "api" / "v1" / "admin" / "stats"     => handler(req)(stats(req))
  }
