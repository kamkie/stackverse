...
final class ModerationRoutes(
  ...
  val routes: HttpRoutes[IO] = HttpRoutes.of[IO] {
    case req @ POST -> Root / "api" / "v1" / "bookmarks" / id / "reports" => handler(req)(createReport(req, id))
    case req @ GET -> Root / "api" / "v1" / "reports"                     => handler(req)(listMyReports(req))
    case req @ PUT -> Root / "api" / "v1" / "reports" / id                => handler(req)(updateMyReport(req, id))
    case req @ DELETE -> Root / "api" / "v1" / "reports" / id             => handler(req)(withdrawReport(req, id))
    case req @ GET -> Root / "api" / "v1" / "admin" / "reports"           => handler(req)(listReports(req))
    case req @ PUT -> Root / "api" / "v1" / "admin" / "reports" / id      => handler(req)(resolveReport(req, id))
    case req @ PUT -> Root / "api" / "v1" / "admin" / "bookmarks" / id / "status" =>
      handler(req)(setBookmarkStatus(req, id))
  }
