...
The Stackverse backend in Go 1.26 with the [Echo](https://echo.labstack.com/)
framework at the HTTP edge, [pgx](https://github.com/jackc/pgx) against
PostgreSQL, and JWT bearer authentication against Keycloak's JWKS. Shared
...
Echo was chosen over Gin and Fiber for this variant because it is a widely used
higher-level Go web framework while staying on `net/http`. That keeps the
comparison against the existing Go chi backend focused on framework routing,
route-level middleware, and error handling, rather than changing the HTTP
runtime as well.
...
Local verification (plain unit tests, no containers):
...
```sh
docker build -t stackverse/backend-go-echo:local -f backends/go-echo/Dockerfile .
```

## What this implementation demonstrates

- **Echo at the HTTP edge** — route registration, route-level middleware
  stacking, path parameters, and framework-level 404/405 handling are
  Echo-owned. The domain handlers stay as plain `http.HandlerFunc`s through a
  small adapter, so the comparison with `backends/go` stays concentrated on the
  framework boundary.
...
- Echo handlers commonly accept `echo.Context` directly. This variant wraps
  feature handlers as `net/http` functions instead, leaving Echo concentrated in
  `internal/app` and the adapter in `internal/web`; that keeps the business
  logic shape directly comparable to the chi sibling while still exercising
