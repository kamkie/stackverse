...
func TestNewRouterPassesEchoPathParamsToHandlers(t *testing.T) {
	handler, _ := New(config.Config{
		IssuerURI: "http://localhost:8180/realms/stackverse",
		JWKSURI:   "http://localhost:8180/realms/stackverse/protocol/openid-connect/certs",
	}, nil, slog.New(slog.NewTextHandler(io.Discard, nil)))
	...
	handler.ServeHTTP(response, httptest.NewRequest(http.MethodGet, "/api/v1/bookmarks/not-a-uuid", nil))

	if response.Code != http.StatusBadRequest {
		t.Fatalf("invalid route UUID status = %d, want 400", response.Code)
	}
	if got := response.Header().Get("Content-Type"); got != "application/problem+json" {
		t.Fatalf("invalid route UUID content type = %q", got)
	}
}
