  ...
  try {
    const result = unwrap(
      await api.GET("/api/v1/admin/reports", {
        params: { query: { status: status.value, page: page.value, size: 20 } },
      }),
    );
...
<template>
  ...
  <p v-else-if="reports.length === 0" class="sv-empty">{{ t("ui.reports.empty") }}</p>
  <template v-else>
      ...
      <table class="sv-table">
        ...
        <tbody>
          <tr v-for="report in reports" :key="report.id" :data-ctx="`report:${report.id}`">
            ...
            <td><span class="sv-badge">{{ statusLabel(report.status) }}</span></td>
