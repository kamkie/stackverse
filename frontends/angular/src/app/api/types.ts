...
export interface BookmarkCursorPage {
  items: Bookmark[];
  /** Opaque cursor for the next slice. Absent on the last page. */
  nextCursor?: string;
}
...
/** Offset-paginated page — same envelope for every v1 list. */
...
export interface FieldError {
  field: string;
  messageKey: string;
  /** Localized per the language resolution rules (`lang` → `Accept-Language` → `en`). */
  message: string;
}

/** RFC 9457 problem details. `errors` carries field-level validation failures. */
