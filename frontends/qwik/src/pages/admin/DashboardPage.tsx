...
export default component$<{ i18n: I18nState; onNavigate$: PropFunction<(path: string) => void> }>((props) => {
  ...
  const maxValue = state.stats
    ? Math.max(
        1,
        ...state.stats.daily.flatMap((day) => [day.bookmarksCreated, day.activeUsers]),
      )
    : 1;
        ...
        <>
          <div class="sv-stats-grid">
            <div class="sv-stat">
              ...
              <span class="sv-stat-label">{m(props.i18n, "ui.admin.stats.users")}</span>
            </div>
            <div class="sv-stat">
              <span class="sv-stat-value">{state.stats.totals.bookmarks}</span>
              <span class="sv-stat-label">{m(props.i18n, "ui.admin.stats.bookmarks")}</span>
            </div>
            <div class="sv-stat">
              <span class="sv-stat-value">{state.stats.totals.publicBookmarks}</span>
              <span class="sv-stat-label">{m(props.i18n, "ui.admin.stats.public-bookmarks")}</span>
            </div>
            <div class="sv-stat">
              <span class="sv-stat-value">{state.stats.totals.hiddenBookmarks}</span>
              <span class="sv-stat-label">{m(props.i18n, "ui.admin.stats.hidden-bookmarks")}</span>
            </div>
            <a
            ...
            >
              <span class="sv-stat-value">{state.stats.totals.openReports}</span>
              <span class="sv-stat-label">
                {mc(props.i18n, "ui.admin.stats.open-reports", state.stats.totals.openReports)}
              </span>
            </a>
          </div>

          <div class="sv-card">
            <h2 class="sv-sidebar-title">{m(props.i18n, "ui.admin.chart.label")}</h2>
            <svg class="sv-chart" viewBox="0 0 620 180" role="img" aria-label={m(props.i18n, "ui.admin.chart.label")}>
            ...
            <div class="sv-legend">
              <span><span class="sv-legend-swatch" />{m(props.i18n, "ui.admin.stats.bookmarks-created")}</span>
              <span><span class="sv-legend-swatch sv-legend-swatch--secondary" />{m(props.i18n, "ui.admin.stats.active-users")}</span>
            </div>
          </div>
        </>
      )}
