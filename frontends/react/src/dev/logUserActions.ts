// Dev-only: logs user interactions and their outcomes to the console —
// clicks, form submits, route changes, and `/api`/`/auth` request results —
// so the console forwarder (forwardConsoleToDevServer.ts) lands them in the
// terminal / dev-server.log and "clicked X" can be matched to the request it
// triggered. Logs element labels and URLs only, never field values
// (docs/LOGGING.md §6); everything goes out at DEBUG.

const MAX_LABEL_LENGTH = 60;

// What counts as "the thing the user clicked": the nearest interactive
// ancestor, so a click on an icon or text inside a button logs the button.
...
// Only same-origin app traffic; keeps the forwarder's own /__client-log
// posts, HMR, and asset requests out of the action log.
...
function describeElement(element: Element): string {
  ...
  // Form fields are described by their static attributes — reading `.value`
  // here would leak user input (and passwords) into the log.
  const label =
    element instanceof HTMLInputElement ||
...
// Rows, cards, and dialogs tag the entity they belong to with
// data-ctx="<type>:<id>" (e.g. report:123), so a "Dismiss" that exists once
// per row still says which row it acted on. Ancestors chain innermost-first.
...
function logClicks() {
  // Capture phase, so handlers that stopPropagation can't hide the click.
  document.addEventListener(
    "click",
    (event) => {
      if (!(event.target instanceof Element)) return;
      const interactive = event.target.closest(INTERACTIVE_SELECTOR);
      // Dead clicks are logged too — "clicked and nothing happened" is
      // exactly what this log exists to make visible.
