<script lang="ts">
  ...
  let q = $state("");
  let page = $state(0);
  let users: Page<UserAccount> | null = $state(null);
  let loading = $state(true);
  let error: Error | null = $state(null);
  let blocking: UserAccount | null = $state(null);
  let reason = $state("");
  let blockError: unknown = $state(undefined);
  let blockReasonError: string | undefined = $state(undefined);
  ...
  function submitBlock(event: SubmitEvent) {
    ...
    if (blocking) {
      void setStatus(blocking.username, "blocked");
    }
  }
</script>

<h1 class="sv-page-title">{m(i18nState.current, "ui.admin.users")}</h1>
<div class="sv-toolbar">
  <input
    type="search"
    class="sv-input"
    placeholder={m(i18nState.current, "ui.users.search.placeholder")}
    bind:value={q}
    oninput={() => {
      page = 0;
      void load();
...
{:else if users}
  <div class="sv-table-wrap">
    <table class="sv-table">
      <thead>
        <tr>
          <th scope="col">{m(i18nState.current, "ui.field.username")}</th>
