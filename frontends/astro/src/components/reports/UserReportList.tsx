...
export function UserReportListContent(props: Props) {
  ...
  const [editReason, setEditReason] = createSignal<ReportReason>(
    REPORT_REASONS[0],
  );
  ...
  return (
    <>
      <div class="sv-toolbar">
        ...
        >
          <option value="">
            {m(i18n(), "ui.my-reports.filter.all-statuses")}
          </option>
      ...
      <Show when={editing()}>
        {(report) => (
          <Dialog
            title={m(i18n(), "ui.my-reports.dialog.edit")}
            ...
            <form class="sv-form" onSubmit={saveEdit}>
              ...
              >
                <select
                  ...
                  value={editReason()}
                  onChange={(event) =>
                    setEditReason(event.currentTarget.value as ReportReason)
                  }
                >
                  <For each={REPORT_REASONS}>
                    {(option) => (
                      <option value={option}>
                        {m(i18n(), `ui.report.reason.${option}`)}
                      </option>
                    )}
                  </For>
                </select>
              ...
              <Show
                when={
                  editError() instanceof ApiError &&
                  (editError() as ApiError).status === 409
                }
              >
                <div class="sv-alert sv-alert--warning" role="alert">
                  {(editError() as ApiError).message}
                </div>
              </Show>
...
export default function UserReportList() {
  ...
  return (
    ...
    >
      ...
      >
        <UserReportListContent toast={island.toast} />
      </Show>
