...
| [PHP Laravel](../backends/php-laravel/README.md) | Eloquent ORM/query builder plus Laravel migrations | Eloquent models/query builders for normal operations; focused raw SQL for PostgreSQL text[], tuple keysets, and aggregates; Laravel migrations | ✅ idiomatic |
...
| [Open Liberty](../backends/open-liberty-java/README.md) | CDI beans with @Inject; beans.xml bean-discovery | Application-scoped config/runtime/log/catalog beans; injected resources and providers; annotation discovery | ✅ idiomatic |
...
| [NestJS](../backends/node-nestjs/README.md) | Passport guards / @UseGuards, AuthGuard, @Roles decorators | Global bearer + route-authorization CanActivate guards use @Public/@Roles metadata; a pre-parser Fastify hook shares the idempotent jose verifier to preserve invalid/blocked-token precedence | 🟡 deliberate |
| [Open Liberty](../backends/open-liberty-java/README.md) | MicroProfile JWT / Jakarta Security declarative role checks | Liberty mpJwt validates tokens; name-bound @RequiresCaller and @RequiresRole filters enforce protected routes before entity deserialization and adapt Keycloak's nested realm_access.roles claim | 🟡 deliberate |
| [PHP Laravel](../backends/php-laravel/README.md) | Guards/Sanctum/Passport or custom auth middleware for APIs | Stateless Laravel request guard backed by firebase/php-jwt/JWK; auth:api plus role middleware, no browser sessions/Sanctum | ✅ idiomatic |
...
| [NestJS](../backends/node-nestjs/README.md) | class-validator/class-transformer DTOs with a global ValidationPipe | Concrete request DTOs with class-validator metadata, class-transformer normalization, and one global contract-aware ValidationPipe | ✅ idiomatic |
| [Open Liberty](../backends/open-liberty-java/README.md) | Bean Validation (@Valid / Hibernate Validator) on inputs | Deliberately invokes the injected Validator after authorization/resource checks to preserve contract error precedence; Stackverse mapper localizes keyed violations | 🟡 deliberate |
| [PHP Laravel](../backends/php-laravel/README.md) | FormRequest / Validator facade with validation rules | Concrete FormRequests normalize bodies and map Laravel rule failures to localized contract FieldViolations | ✅ idiomatic |
...
| [Spring Boot (Java)](../backends/spring-java/README.md) | JUnit 5, @SpringBootTest/MockMvc, Testcontainers, spring-security-test | JUnit 5 service units plus MockMvc controller/security slices; live PostgreSQL API behavior remains in conformance | 🟡 deliberate |
| [Ktor (Kotlin)](../backends/ktor-kotlin/README.md) | testApplication from ktor-server-test-host with kotlin.test/JUnit5 | testApplication + JUnit 5 units and Testcontainers PostgreSQL coverage of the real Ktor/Flyway/JDBC module | ✅ idiomatic |
...
| [Elixir Phoenix](../backends/elixir-phoenix/README.md) | ExUnit with ConnCase/DataCase and focused unit tests | Authenticated ConnCase HTTP tests, DataCase units, PostgreSQL context/seed regressions, and logging/privacy checks | ✅ idiomatic |
| [Go (chi)](../backends/go/README.md) | stdlib testing, table-driven tests, *_test.go beside code | Table-driven units plus PostgreSQL-backed full-router/JWKS contract tests; gotestsum wraps for JUnit in CI | ✅ idiomatic |
| [Go (Echo)](../backends/go-echo/README.md) | stdlib testing, table-driven tests, httptest for router edges | Table-driven units, Echo/JWKS boundary tests, and a PostgreSQL-backed router contract suite | ✅ idiomatic |
...
| [Micronaut](../backends/micronaut-java/README.md) | @MicronautTest with injected HTTP client for integration tests | JUnit/AssertJ units, @MicronautTest HTTP boundaries, and Testcontainers PostgreSQL persistence coverage | ✅ idiomatic |
| [Node (Fastify/TS)](../backends/node-ts/README.md) | Vitest/Jest, unit plus integration (supertest/inject) tests | Vitest units plus Fastify injection for auth, message, moderation, ETag, and problem boundaries; no live DB | ✅ idiomatic |
...
| [Open Liberty](../backends/open-liberty-java/README.md) | JUnit 5 units; Arquillian/liberty-maven integration tests | JUnit units plus Testcontainers PostgreSQL resource/filter/runtime coverage and Failsafe verification of the minimized Liberty package | ✅ idiomatic |
| [PHP Laravel](../backends/php-laravel/README.md) | PHPUnit or Pest through `php artisan test`, often with HTTP feature tests | PHPUnit units/HTTP features plus locally opt-in, CI-default PostgreSQL contract suites | ✅ idiomatic |
...
| [Django + DRF](../backends/python-django/README.md) | pytest/pytest-django or Django TestCase with APIClient integration tests | pytest-django + DRF APIClient over real PostgreSQL, with focused startup/auth/logging units | ✅ idiomatic |
| [Play (Scala)](../backends/play-scala/README.md) | ScalaTestPlusPlay with GuiceApplicationBuilder for controller/app tests | Guice/ScalaTest units plus Testcontainers PostgreSQL coverage of Flyway, routed controllers, JDBC, and transactions | ✅ idiomatic |
...
| [NestJS](../backends/node-nestjs/README.md) | DTO classes + entities; enums; @nestjs/swagger OpenAPI generation | Decorated request DTO classes; raw-pg row interfaces, string-literal wire unions, and omitNulls remain explicit | 🟡 deliberate |
| [Open Liberty](../backends/open-liberty-java/README.md) | Typed DTOs bound by JSON-B/Jackson, often OpenAPI-generated | Typed request and response records; Jackson renders deterministic JSON; maps limited to open audit detail/internal assembly | ✅ idiomatic |
| [PHP Laravel](../backends/php-laravel/README.md) | Eloquent API resources / JsonResource DTOs or arrays | Unwrapped JsonResource classes preserve lowercase wire strings and omit optional nulls | ✅ idiomatic |
...
| [Fastify](../gateways/node-fastify/README.md) | Vitest or node:test running Fastify in-process with injected requests | Vitest hosts `buildApp` in process and covers deterministic upstreams, RSA/JWKS, Redis-store, logging/OTLP, security, and lifecycle boundaries | ✅ idiomatic |
...
| [Rust Axum](../gateways/rust/README.md) | cargo test with tower::ServiceExt and local stub services | Axum in-process requests plus backend/IdP/RESP fixtures and spawned-binary startup/OTLP/privacy tests | ✅ idiomatic |
...
| [Astro](../frontends/astro/README.md) | Static `.astro` pages composed through layouts, with selectively hydrated islands or an external client entry for fully interactive content | Nine static Astro pages share one layout and directly hydrate focused Solid components | ✅ idiomatic |
...
| [Astro](../frontends/astro/README.md) | File-based routes under `src/pages`, with props and slots composing shared layouts | Nine generated routes compose content through `BaseLayout.astro`'s slot and use native navigation; no client router | ✅ idiomatic |
...
| [Astro](../frontends/astro/README.md) | `src/pages` for routes, `src/layouts` for shared documents, and focused client entries/components where interactivity is required | Slotted file-based pages plus `BaseLayout.astro`; each page directly imports its interactive Solid component | ✅ idiomatic |
...
| [SolidJS](../frontends/solid/README.md) | Vitest + @solidjs/testing-library for component tests | Vitest + jsdom helpers and Testing Library form/component/page interactions; shared e2e remains the full browser gate | ✅ idiomatic |
