...
- Token refresh is the gateway's job — transparent to both SPA and backend. Its two
  ...
  refresh (token expired or revoked), the session is dead — destroy it and degrade
  the request to anonymous. When the IdP is **unreachable**, nothing is known about
  the session — keep it (the refresh token may still be valid) and fail the request
  with a `503` problem document. A transient IdP outage must not log anyone out.
